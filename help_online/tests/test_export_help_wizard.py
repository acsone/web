...
class TestExportHelpWizard(TestWizardCommon):
    ...
    def test_export_help(self):
        ...
        self.assertEqual("website." + self.pageName, templateNodeList[0].attrib["key"])

        if self.imgXmlId:
            xPath = ".//record[@id='%s']" % self.imgXmlId
            imgNodeList = rootXml.findall(xPath)
            self.assertEqual(
                len(imgNodeList), 1, "The same image should be exported only once"
            )

            for imgElem in templateNodeList[0].iter("img"):
                imgSrc = imgElem.get("src")
                if "/ir.attachment/" in imgSrc:
                    self.assertIn("/ir.attachment/%s|" % self.imgXmlId, imgSrc)
                else:
