...
class TestWizardCommon(object):
    ...
    def createPage(self, pageName, imgXmlId=False):
        ...
        tNode = ET.SubElement(rootNode, "t", attrib={"t-call": "website.layout"})
        structDivNode = ET.SubElement(
            tNode, "div", attrib={"class": "oe_structure oe_empty", "id": "wrap"}
        )
        sectionNode = ET.SubElement(
            structDivNode, "section", attrib={"class": "mt16 mb16"}
        )
        containerNode = ET.SubElement(sectionNode, "div", attrib={"class": "container"})
        rowNode = ET.SubElement(containerNode, "div", attrib={"class": "row"})
        bodyDivNode = ET.SubElement(
            rowNode, "div", attrib={"class": "col-md-12 " "text-center mt16 mb32"}
        )
        style = (
        ...
        h2Node = ET.SubElement(bodyDivNode, "h2", attrib={"style": style})
        h2Node.text = "Test Sample Title"
        if imgId:
            imgDivNode = ET.SubElement(
                bodyDivNode, "div", attrib={"style": "text-align: left;"}
            )
            src = "/website/image?field=datas&" "model=ir.attachment&id=%s" % str(imgId)
            ET.SubElement(
                imgDivNode, "img", attrib={"class": "img-thumbnail", "src": src}
            )
            imgDivNode = ET.SubElement(
                bodyDivNode, "div", attrib={"style": "text-align: left;"}
            )
