...
HELP_ONLINE_SNIPPET_IMAGE_PATH = (
...
class ExportHelpWizard(models.TransientModel):
    ...
    def _manage_images_on_page(self, page_node, data_node, exported_resources):
        ...
        for img_elem in page_node.iter("img"):
            ...
            if urls.test(parse_result.path, "GET"):
                endpoint, kwargs = urls.match(path, "GET", query_args=query_args)
                ...
                if not image or not image.exists() or image._name != img_model:
                    raise exceptions.UserError(
                        _(
                            "Only images from ir.attachment are supported when "
                            "exporting help pages"
                        )
                    )
                exported_data = image.export_data(
                    ["id", "datas", "datas_fname", "name", "res_model", "mimetype"],
                    raw_data=False,
                )["datas"][0]
                ...
                img_node = ET.SubElement(
                    data_node, "record", attrib={"id": xml_id, "model": image._name}
                )
                field_node = ET.SubElement(img_node, "field", attrib={"name": "datas"})
                field_node.text = str(exported_data[1])
                field_node = ET.SubElement(
                    img_node, "field", attrib={"name": "datas_fname"}
                )
                field_node.text = exported_data[2]
                field_node = ET.SubElement(img_node, "field", attrib={"name": "name"})
                field_node.text = exported_data[3]
                field_node = ET.SubElement(
                    img_node, "field", attrib={"name": "res_model"}
                )
                field_node.text = exported_data[4]
                field_node = ET.SubElement(
                    img_node, "field", attrib={"name": "mimetype"}
                )
    ...
    def _generate_snippet_from_template(self, page_node, template_id, template_prefix):
        ...
        xpath = ET.SubElement(
            snippet,
            "xpath",
            attrib={"expr": "//div[@id='snippet_structure']", "position": "inside"},
        )
        main_div = ET.SubElement(xpath, "div")
        thumbnail = ET.SubElement(
            main_div, "div", attrib={"class": "oe_snippet_thumbnail"}
        )
        ET.SubElement(
            thumbnail,
            "img",
        ...
        span = ET.SubElement(
            thumbnail, "span", attrib={"class": "oe_snippet_thumbnail_title"}
        )
        span.text = page_node.attrib["name"].replace(template_prefix, "")
        body = ET.SubElement(
            main_div,
            "section",
            attrib={"class": "oe_snippet_body " "mt_simple_snippet"},
        )
    ...
    def _get_qweb_views_data(self):
        ...
        template_prefix = parameter_model.get_param(TEMPLATE_PREFIX_PARAMETER, False)
        ...
        for ir_ui_view in ir_ui_views:
            ...
            root.tag = "template"
            xml_id = self._get_ir_ui_view_xml_id(ir_ui_view, root.attrib.pop("t-name"))
        ...
        if len(ir_ui_views) > 0:
            return ET.tostring(
                xml_to_export, encoding="utf-8", xml_declaration=True, pretty_print=True
            )
        else:
            return False
    ...
    def _get_ir_ui_view_xml_id(self, ir_ui_view, template_name):
        ...
        data = ir_model_data.search(
            [("model", "=", ir_ui_view._name), ("res_id", "=", ir_ui_view.id)]
        )
        if data:
            if data[0].module:
    ...
    def auto_backup(self):
        ...
        """
        parameter_model = self.env["ir.config_parameter"]
        autobackup_path = parameter_model.get_param(AUTOBACKUP_PARAMETER, False)

        if autobackup_path:
            xml_data = self._get_qweb_views_data()
            try:
                timestr = time.strftime("%Y%m%d-%H%M%S")
                filename = "%s/help_online_backup-%s.xml" % (autobackup_path, timestr)
