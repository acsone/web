...
class WebForm(models.Model):
    ...
    mail_template_partner_field_id = fields.Many2one(comodel_name="ir.model.fields")
    ...
    def _onchange_mail_template_model_id(self):
        ...
        if (
            not self.mail_template_model_id
            or self.mail_template_model_id.model == "res.partner"
        ):
            domain = [("id", "=", -1)]
        else:
            partner_fields = self.mail_template_model_id.field_id.filtered(
                lambda f: f.ttype == "many2one" and f.relation == "res.partner"
            )
            domain = [("id", "in", partner_fields.ids)]
        return {"domain": {"mail_template_partner_field_id": domain}}
        ...
        for rec in self.filtered(
            ...
            if rec.mail_template_model_id.model == "res.partner":
                rec.url_mail_template = (
                    ...
                    "<code>${object.form_token}</code>"
                    "/<code>${object.id}</code>"
                )
            else:
                url = (
                    f"/web_form/<strong>{rec.code}</strong>/"
                    f"<code>${{object.{rec.mail_template_partner_field_id.name,}"
                    f".form_token}}</code>"
                    f"/<code>${{object.{rec.mail_template_partner_field_id.name}"
                    f".id}}</code>"
                )
